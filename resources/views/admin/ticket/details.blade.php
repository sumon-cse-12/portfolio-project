...
@section('title') {{trans('admin.ticket_details')}} @endsection
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h2 class="card-title">{{trans('customer.ticket_details')}}</h2>
                        <div class="float-right">

                        </div>
                    </div>
                    <!-- /.card-header -->
                    <form action="{{route('admin.store')}}" id="messageForm" method="post" enctype="multipart/form-data">
                        @csrf
                                                        ...
                                                        @if($conversation->document)
                                                            <div class="d-block float-right">
                                                                <div class="document_image" style="margin-left: 45px;">
                                                                    <a target="_blank" href="{{route('admin.ticket.download',['id'=>$conversation->id,'file'=>$conversation->document])}}">
                                                                        <img src="{{asset('uploads/'.$conversation->document)}}" alt="">
                                                                    </a>
                                                                </div>
                                                            </div>
                                                        @endif
...
@section('extra-scripts')
    <script>
        ...
        $('.fa-times').on('click', function (e){
            location.href='{{route('admin.ticket.index')}}';
        });
    </script>
@endsection

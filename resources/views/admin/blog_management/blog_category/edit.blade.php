        ...
        <div class="col-12 mx-auto col-sm-10 mt-3">
            <div class="card">
                <div class="card-header">
                    <h2 class="card-title">{{ trans('admin.blog') }} {{ trans('admin.category') }}  {{ trans('admin.edit') }} </h2>
                    <a class="btn btn-info float-right" href="{{ route('admin.blog-category.index') }}">{{ trans('admin.back') }}</a>
                </div>
                <form method="post" role="form" id="blogCategoryForm" action="{{ route('admin.blog-category.update',[$blog_category]) }}">
                    @csrf
                    @method('PUT')
                    <div class="card-body">

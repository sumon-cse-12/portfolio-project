...
class TicketController extends Controller
{
    ...
    {
        $customers = auth()->user()->tickets()->select(['id', 'subject','customer_id','status'])->orderByDesc('created_at');
        return datatables()->of($customers)
            ...
            ->addColumn('status', function ($q) {
                if ($q->status=='pending'){
                    ...
                    return '<button type="button" class="btn light btn-sm btn-info dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                                                Open
                               </button>
                                <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 38px, 0px);">
                                     <button data-message="Are you sure, you want to processing this ticket?" data-action=' . route('admin.ticket.status', ['id' => $q->id, 'status' => 'processing']) . '
                ...
                if ($q->status=='solved'){
                    return "<a class='btn btn-sm btn-info disabled' href='#' title='Reply'><i class='fa fa-reply'></i></a> &nbsp; &nbsp;";
                }else {
                    return "<a target='_blank' class='btn btn-sm btn-info' href='" . route('admin.ticket.reply', ['id' => $q->id]) . "' title='Reply'><i class='fa fa-reply'></i></a> &nbsp; &nbsp;";
                }
    ...
    public function status(Request $request){
        ...
        return redirect()->route('admin.ticket.index')->with('success', trans('customer.message.status_changes'));
    }
